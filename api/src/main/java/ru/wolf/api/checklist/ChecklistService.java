...
public class ChecklistService {
    ...
    @GetMapping @Transactional(readOnly=true) public List<Response> list(Authentication a,LocalDate date){return items.findForDate(user(a),date).stream().map(this::response).toList();}
    ...
    @PatchMapping("/{id}") @Transactional public Response update(Authentication a,Long id,Update r){DailyChecklistItem i=owned(a,id);if(r.title()!=null)i.setTitle(r.title().trim());if(r.position()!=null)i.setPosition(r.position());if(r.done()!=null){i.setDone(r.done());i.setDoneAt(r.done()?Instant.now():null);}return response(items.save(i));}
    @DeleteMapping("/{id}") @Transactional public ResponseEntity<Void> delete(Authentication a,Long id){items.delete(owned(a,id));return ResponseEntity.noContent().build();}
    @PostMapping("/{id}/carry-over") @Transactional public Response carry(Authentication a,Long id,Carry r){DailyChecklistItem o=owned(a,id);DailyChecklistItem copy=DailyChecklistItem.builder().user(o.getUser()).date(r.toDate()).title(o.getTitle()).delo(o.getDelo()).position(0).done(false).build();return response(items.save(copy));}
