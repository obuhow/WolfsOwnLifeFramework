...
public class FocusReviewService {
    ...
    public ReviewResponse review(String username, Long id) {
        FocusSession session = ownedSession(username, id);
    ...
    @Transactional(readOnly = true)
    public PreviewResponse preview(String username, Long id, AllocationRequest request) {
        FocusSession session = ownedSession(username, id);
        Plan plan = plan(session, username, request.allocations(), false);
        return plan.preview();
    }
    @Transactional
    public PreviewResponse apply(String username, Long id, AllocationRequest request) {
        FocusSession session = ownedSession(username, id);
        Plan plan = plan(session, username, request.allocations(), true);
        ...
        Map<LocalDateTime, Delo> targetBySlot = plan.changes().stream().collect(java.util.stream.Collectors.toMap(Change::slot, c -> delo(username, c.targetDeloId())));
        ...
        for (Allocation allocation : request.allocations()) {
            ...
            if (allocation.minutes() != null && allocation.minutes() > 0) {
                if (allocation.deloId() != null) distraction.setDelo(delo(username, allocation.deloId()));
    ...
    @Transactional
    public PreviewResponse revert(String username, Long id) {
        FocusSession session = ownedSession(username, id);
    ...
    private Plan plan(FocusSession session, String username, List<Allocation> allocations, boolean apply) {
        ...
        for (Allocation allocation : allocations) {
            ...
            Delo target = delo(username, allocation.deloId() == null ? (distraction.getDelo() == null ? null : distraction.getDelo().getId()) : allocation.deloId());
    ...
    private FocusSession ownedSession(String username, Long id) { 
    ...
    private Delo delo(String username, Long id) { return id == null ? null : delos.findByUserAndId(current(username), id).orElseThrow(); }
