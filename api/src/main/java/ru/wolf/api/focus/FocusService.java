...
public class FocusService {
    ...
    @Transactional
    public ResponseEntity<?> start(Authentication auth, StartRequest request) {
    ...
    @Transactional(readOnly = true)
    public ResponseEntity<?> details(Authentication auth, Long id) {
        User user = current(auth);
    ...
    @Transactional
    public ResponseEntity<Void> deleteDistraction(Authentication auth, Long sessionId, Long distractionId) {
        User user = current(auth);
    ...
    @Transactional
    public ResponseEntity<?> stop(Authentication auth, Long id, StopRequest request) {
        User user = current(auth);
        FocusSession session = sessions.findById(id).filter(item -> item.getUser().getId().equals(user.getId())).orElseThrow();
    ...
    @Transactional
    public ResponseEntity<?> distraction(Authentication auth, Long id, DistractionRequest request) {
        User user = current(auth);
        FocusSession session = sessions.findById(id).filter(item -> item.getUser().getId().equals(user.getId())).orElseThrow();
