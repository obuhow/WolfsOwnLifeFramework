...
/**
 * Test double for {@link MaxPort}. Records every outbound call so unit tests can
 * assert isolation (which chat got which message) without touching the real Bot
 * API. Active only under the {@code test} profile.
 */
@Component
@Profile("test")
@RequiredArgsConstructor
public class FakeMaxAdapter implements MaxPort {

    private final MaxMessageCollector collector;
    ...
    @Component
    @RequiredArgsConstructor
    static class MaxMessageCollector {
        @Getter
