...
public class GanttService {
    ...
    public WeekPlanResponse upsertWeekPlan(
        ...
        BigDecimal hours = request.planHours();
    ...
    private Map<String, BigDecimal> computeFacts(User user, LocalTime dayEnd, List<WeekColumn> weeks) {
        ...
        for (TimeEntry entry : entries) {
            ...
            for (WeekColumn week : weeks) {
                LocalDate monday = LocalDate.parse(week.weekStart());
                LocalDate nextMonday = LocalDate.parse(week.weekEndExclusive());
                ...
                String weekKey = weekKey(week.isoYear(), week.isoWeek());
    ...
    private Map<String, BigDecimal> loadPlans(User user, WeekColumn first, WeekColumn last) {
        List<WeekPlan> plans = weekPlanRepository.findInWeekRange(
                user,
                first.isoYear(), first.isoWeek(),
                last.isoYear(), last.isoWeek()
        );
    ...
    private ProjectRow toRow(
        ...
        for (WeekColumn w : weeks) {
            String ck = cellKey(p.getId(), weekKey(w.isoYear(), w.isoWeek()));
