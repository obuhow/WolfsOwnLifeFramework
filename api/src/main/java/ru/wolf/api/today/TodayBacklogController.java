...
public class TodayBacklogController {
    ...
    public ResponseEntity<BacklogResponse> backlog(Authentication auth, @RequestParam String date) {
        User user = current(auth); LocalDate day = LocalDate.parse(date); WeekFields wf = WeekFields.ISO;
        ...
        return ResponseEntity.ok(new BacklogResponse(weekId, result, totalPlan, totalFact));
    }
