...
**Status:** ready-for-agent (решение И-D изменено владельцем; конфликтный контур требует переработки)

- [ ] Конфликт = **хотя бы одна 15-минутная ячейка импортируемого интервального блока
      пересекается** с любой существующей Записью времени (без сравнения активностей)
...
- [ ] «Пропустить все» (SKIP_ALL, дефолт) — конфликтующие интервальные блоки не трогаются,
      неконфликтующие применяются без дробления на ячейки
- [ ] «Перезаписать все» (OVERWRITE_ALL) — пересекающиеся существующие интервалы заменяются
      данными из файла без дублей и без потери незатронутых частей
- [x] «Отменить» (CANCEL) — ничего не пишется, `importRunId = null`
- [x] Поячеечных вопросов по конфликтам НЕ вводить (осознанно вне объёма)
- [ ] `*ApiIT`: импорт поверх занятых интервалов для каждой из трёх веток (skip/overwrite/cancel)
      даёт ожидаемое состояние; без дублей и дробления непрерывного блока

## Decision update

Импорт больше не материализует отдельную строку на каждую ячейку. Preview и apply должны
сначала построить интервальные блоки, а затем проверять пересечения блока с существующими
Записями. Стратегии skip/overwrite/cancel сохраняются.
...
Эти результаты относятся к прежней модели «одна конфликтующая ячейка = одна строка» и
требуют повторной проверки после перехода на интервальные блоки.
