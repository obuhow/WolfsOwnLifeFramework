# WOLF Release 0.1 — Календарная ОС

Status: `ready-for-agent`  
Feature slug: `release-0.1-calendar-os`
...
- One-shot импорт CSV (Дела) **(ICS отменён — перенесён в release-0.4-cancelled-demo-sandbox)**
...
58. As a Пользователь, I want to upload an ICS once, so that existing calendar events become Записи времени. **(cancelled — moved to release-0.4-cancelled-demo-sandbox)**
59. As a Пользователь, I want long ICS events split/rounded onto 15-minute cells, so that they fit the grid model. **(cancelled — moved to release-0.4-cancelled-demo-sandbox)**
...
- **Import** — CSV → Дела; **ICS → Записи времени (cancelled, moved to release-0.4-cancelled-demo-sandbox)**; 15-min normalization rules.
...
- Commands: confirm-all, apply-recurrence, import CSV. **(import ICS cancelled — moved to release-0.4-cancelled-demo-sandbox)**
...
- CSV import observable results via subsequent GETs. **(ICS import cancelled — moved to release-0.4-cancelled-demo-sandbox)**
...
- Import file schemas (exact CSV columns) may be fixed during implementation and documented beside import code; behaviour must still meet stories 56–57. **(ICS stories 58-59 cancelled — moved to release-0.4-cancelled-demo-sandbox)**
- After this spec, split work with `to-tickets` into `.scratch/release-0.1-calendar-os/issues/NN-*.md` rather than coding from the whole release at once.
