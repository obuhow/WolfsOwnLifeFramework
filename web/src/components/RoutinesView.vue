...
<template>
  <section class="page routines-page">
    ...
    <div v-else class="routine-list">
      <article v-for="item in routines" :key="item.id" class="routine-row" :class="{ selected: selectedId === item.id, archived: item.archived }" @click="selectedId = item.id">
