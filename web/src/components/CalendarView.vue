...
const days = ref([]); const loading = ref(false); const error = ref('')
...
async function loadMonth() { syncUrl(); if (view.value !== 'month') return; loading.value = true; try { const res = await fetch(`${apiBase()}/calendar/month?month=${month.value}`, { headers: authHeaders() }); if (!res.ok) throw new Error(`Календарь: HTTP ${res.status}`); const body = await res.json(); days.value = body.days || [] } catch (e) { error.value = e.message } finally { loading.value = false } }
