...
async function addMetric() {
  try { await request(`/goals/${selectedId.value}/metrics`, { method: 'POST', body: JSON.stringify({ kind: metric.value.kind, value: Number(metric.value.value), targetValue: metric.value.targetValue === '' ? null : Number(metric.value.targetValue), at: metric.value.at ? `${metric.value.at}T12:00:00` : null }) }); metric.value = { kind: '', value: '', targetValue: '', at: '' }; await loadDetail() }
  catch (e) { error.value = e.message }
}
async function incrementMetric(item, amount) {
  try {
    await request(`/goals/${selectedId.value}/metrics/${item.id}/increment`, { method: 'PATCH', body: JSON.stringify({ amount }) })
    success.value = `${item.kind}: ${amount > 0 ? '+' : ''}${amount}`
    await loadDetail()
  } catch (e) { error.value = e.message }
}
  ...
  <section class="page goals-page">
    ...
    <div v-if="selected" class="goal-detail">
      ...
      <div class="goal-grid">
        ...
        <section class="detail-section"><h2>Метрики</h2><div class="inline-form"><label>Тип<input v-model="metric.kind" placeholder="вес кг / доход ₽"></label><label>Значение<input v-model="metric.value" type="number" step="any"></label><label>Цель<input v-model="metric.targetValue" type="number" min="0" step="any" placeholder="необязательно"></label><label>Дата<input v-model="metric.at" type="date"></label><button class="btn btn-primary" @click="addMetric">Добавить</button></div><ul class="history"><li v-for="item in selected.metrics" :key="item.id"><span>{{ item.kind }}</span><div class="metric-actions"><button class="metric-quick" @click="incrementMetric(item, 1)">+1</button><button class="metric-quick" @click="incrementMetric(item, 5)">+5</button><input class="metric-custom" type="number" step="any" placeholder="± значение" @keyup.enter="incrementMetric(item, Number($event.target.value))"><strong>{{ item.value }}<small v-if="item.targetValue != null"> / {{ item.targetValue }}</small></strong><time>{{ item.at?.slice(0, 10) }}</time></div></li><li v-if="!selected.metrics.length" class="muted">История пока пуста.</li></ul></section>
...
.notice { padding:10px 0; margin:0 0 12px; } .notice-error { color:var(--wolf-ink); } .notice-success { color:var(--wolf-done-ink); }
...
.history { list-style:none; padding:0; margin:20px 0 0; border-top:1px solid var(--wolf-rule); } .history li { display:flex; justify-content:space-between; align-items:center; gap:12px; padding:10px 0; border-bottom:1px solid var(--wolf-subrule); } .history time, .muted { color:var(--wolf-muted); font-size:12px; } .metric-actions { display:flex; align-items:center; gap:8px; flex-wrap:wrap; } .metric-quick { border:1px solid var(--wolf-rule); background:transparent; padding:4px 8px; cursor:pointer; } .metric-custom { width:92px; } .metric-actions small { color:var(--wolf-muted); font-size:12px; }
